...
void solve(){
  ...
  while(true){
    auto itr=monst1.rbegin();
    if(itr->second==0){
      break;
    }
    auto itr2 = monst2.begin();
    // debug(*itr)
    // debug(*itr2)
    int x = (itr->first-itr2->first);
    int y = -x;
    debug(x)
    debug(y)
    if(x>0){
    ...
    monst1[itr->first]--;
    if(monst1[itr->first]==0){
      monst1.erase(itr->first);
    }
    monst2[itr2->first]--;
    if(monst2[itr2->first]==0){
      monst2.erase(itr2->first);
    }
    if(monst1.size()==0){
      break;
    }
    ...
    auto it = monst2.rbegin();
    if(it->second==0){
      break;
    }
    auto it2 = monst1.begin();
    // debug(*it)
    // debug(*it2)
    int x1 = (it->first-it2->first);
    int y1 = -x1;
    debug(x1)
    debug(y1)
    if(x1>0){
    ...
    monst2[it->first]--;
    if(monst2[it->first]==0){
      monst2.erase(it->first);
    }
    monst1[it2->first]--;
    if(monst1[it2->first]==0){
      monst1.erase(it2->first);
    }
    if(monst1.size()==0){
      break;
    }
  ...
  debug(monst1)
  debug(monst2)
  if(monst1.size()==0 && monst2.size()==0){
    cout<<"Draw"<<endl;
    return;
  }
  if(monst1.size()>monst2.size()){
    cout<<"Tsondu"<<endl;
    return;
  }
  if(monst1.size()<monst2.size()){
