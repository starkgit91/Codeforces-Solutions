...
void solve(){
  ...
  while(i<qry.size()){
    ...
    // debug(hp)
    dp.insert(dp.end(),hp.rbegin(),hp.rend());
    hp.clear();
  }

  // pair<int,int> maxi= qry[qry.size()-1];
  // vector<pair<int,int>>::iterator lower;
  // lower = lower_bound(all(qry),maxi);
  // debug(*lower)
  // for(int i=0;i<qry.size();i++){
  //   dp.pb(qry[])
  // }
