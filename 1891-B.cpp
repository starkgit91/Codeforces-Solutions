...
void solve(){
  ...
  int ans=0;
  int h=2;
  // vector<int> v = primeFactors(56);
  // debug(v)
  // google(n);
  // vector<vector<int>> mat(n,vector<int>(m,0));
  vector<int>vec2(q);
  loop(i,0,q,1) cin>>vec2[i];
  // fsort(vec2);
  ...
  // }
  // reverse(all(vec3));
  loop(i,0,vec3.size(),1){
    int x = vec3[i];
    int x_sqr = 1<<x;
    for(int i=0;i<query.size();i++){
      if(query[i].first%x_sqr==0){
        query[i].first+=(x_sqr/2);
      }
      else if(query[i].first<x_sqr){
        break;
      }
    }
  }
  ...
  map<int,int> hmap;
  loop(i,0,n,1){
    hmap.insert({query[i].second,query[i].first});
  }
