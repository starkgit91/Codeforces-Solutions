...
int maxkjump(int i,vector<int> &vec,vector<int> &dp){
  //memoization
  int n = vec.size();
  if(i>n){
    return 0;
  ...
  dp[i] = maxkjump(i+vec[i-1],vec,dp)+vec[i-1];
  return dp[i];
}


int maxjumpdp(vector<int> &vec){
  //DP tabulation
  int n = vec.size();
  int count=0;
  vector<int> dp(n+1,0);
...
void solve(){
  ...
  // vector<int> dp(n+1,-1);
  // for(int i=1;i<=n;i++){
  //   int j=maxkjump(i,vec,dp);
  //   count = max(count,j);
  // }
  // debug(dp)
  // cout<<count<<endl;
  cout<<maxjumpdp(vec)<<endl;
