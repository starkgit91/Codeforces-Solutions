...
void solve(){
  ...
  int maxi=maxx(cost);
  loop(i,0,n,1){
    if(cost[p-1]<cost[q-1]){
      if(sum+cost[q-1]>count+cost[p-1] || cost[q-1]==maxi){
        count=sum;
        p=q;
      }
    }
    if(cost[s-1]<cost[r-1]){
      if(sum2+cost[r-1]>cnt+cost[s-1] || cost[r-1]==maxi){
        cnt=sum2;
        s=r;
      }
    }
