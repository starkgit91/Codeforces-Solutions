...
pair<int, pair<int,int>> maxSubArraySum(vector<int> &vec){
  int sum = INT64_MIN, fptr = 0, sptr = 0, e = 0, s = 0;
  loop(i, 0, vec.size(),1) {
    fptr += vec[i];
    if (sum < fptr) {
      sum = fptr;
...
void solve(){
  ...
  pair<int,int> coord = {query[a-1].ff.ff,query[a-1].ff.ss};
  pair<int,int> dest ={query[b-1].ff.ff,query[b-1].ff.ss};
  debug(coord)
  debug(dest)
  fsort(query);
  debug(query)
  ...
  int end=0;
  loop(i,0,n,1){
    if(query[i].ff.ff==coord.ff && query[i].ff.ss==coord.ss){
      start=i;
      break;
    }
  }
  loop(i,0,n,1){
    if(query[i].ff.ff==dest.ff && query[i].ff.ss==dest.ss){
      end=i;
      break;
    }
  }
  ...
  for(int i=0;i<n;i++){
    if(query[i].ss==1){
      int x = abs(coord.ff-query[i].ff.ff) + abs(coord.ss-query[i].ff.ss);
      int y = abs(query[i].ff.ff-dest.ff) + abs(query[i].ff.ss-dest.ss);
      count=min(count,x);
      cnt=min(cnt,y);
    }
  }
  ...
  debug(cnt)
  debug(count)
  int cmp = abs(coord.ff-dest.ff) + abs(coord.ss-dest.ss);
  if(query[start].ss==1 && query[end].ss==1){
    cmp=0;
  }
  cout<<min(count,cmp)<<endl;
  // cout<<"mk";
