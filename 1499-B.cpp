...
void solve(){
  ...
  if(s.find(s2)!=string::npos || (s.find("11")!=string::npos && s.find("00")!=string::npos && s.find("11")<s.find("00"))){
    cout<<"NO"<<endl;
    return;
  }
  string f = s;
  while(f.find(s1)!=string::npos){
      int x = f.find(s1);
      debug(x);
      f=f.substr(x+4,f.size()-x-4);
      debug(f)
  }
  if(f!=s){
    True(f.find("00")==string::npos);
    return;
  }
  cout<<"YES"<<endl;
