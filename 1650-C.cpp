...
void solve(){
  ...
  sort(all(query),[&](auto left,auto right){return left.ss<right.ss;});
  debug(query)
  for(int i=0;i<2*n;i++){
    hmaps[query[i].ff]=query[i].ss;
    count+=hmaps[query[i].ff];
  }
  debug(hmaps)
  debug(imap)
  cout<<count<<endl;
  map<int,int>::iterator s,f;
  s=hmaps.begin();
  f=hmaps.end();
  f--;
  int c=0;
  while(c<n){
    cout<<imap[s->ff]<<" "<<imap[f->ff]<<endl;
    s++;
    f--;
    c++;
  }
