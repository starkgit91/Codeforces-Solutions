...
void solve(){
  ...
  vector<pair<int,int>> v2(n,make_pair(0,0));
  vector<pair<int,int>> v3(n,make_pair(0,0));
  loop(i,0,n,1){cin>>v1[i].ff;v1[i].ss=i+1;}
  loop(i,0,n,1){cin>>v2[i].ff;v2[i].ss=i+1;}
  loop(i,0,n,1){cin>>v3[i].ff;v3[i].ss=i+1;}
  fsort(v1);
  fsort(v2);
  fsort(v3);
  loop(i,n-3,n,1){
    loop(j,n-3,n,1){
      loop(k,n-3,n,1){
        if(v1[i].ss!=v2[j].ss && v1[i].ss!=v3[k].ss && v2[j].ss!=v3[k].ss ){
          count=max(count,v1[i].ff+v2[j].ff+v3[k].ff);
        }
      }
    }
  }
