...
void solve(){
  ...
  set<pair<int,int>> query2;
  query.insert({xk-n,yk+k});
  query.insert({xk+n,yk+k});
  query.insert({xk+n,yk-k}); 
  query.insert({xk-n,yk-k}); 
  query.insert({xk-k,n+yk});
  query.insert({xk-k,yk-n}); 
  query.insert({xk+k,yk+n});
  query.insert({xk+k,yk-n}); 
  query2.insert({xq-n,yq+k});
  query2.insert({xq+n,yq+k});
  query2.insert({xq+n,yq-k}); 
  query2.insert({xq-n,yq-k}); 
  query2.insert({xq-k,n+yq});
  query2.insert({xq-k,yq-n}); 
  query2.insert({xq+k,yq-n});
  query2.insert({xq+k,yq+n}); 
  
  debug(query)
  for(auto it:query){
    if(query2.find(it)!=query2.end()){
      count++;
    }
  }
  debug(count)
