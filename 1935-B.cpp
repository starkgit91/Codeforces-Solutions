...
//   loop(i,0,n) cin>>query[i].first>>query[i].second;
...
//   int count=0;
  ...
  vector<int> suff(n);
  for(int i=0; i<n; i++){
    if(hashmap.find(vec[i]) != hashmap.end()) hashmap.erase(vec[i]);
    pref[i] = (*hashmap.begin());
  }
  for(int i=n-1; i>=0; i--){
    if(hmap.find(vec[i]) != hmap.end()) hmap.erase(vec[i]);
    suff[i]=(*hmap.begin());
  }
  debug(pref)
  debug(suff)
