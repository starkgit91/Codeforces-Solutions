...
void solve(){
  ...
  for(int i=n-1;i>0;i--){
    if(vec[i]>=vec[i-1] && flag2==false){
      dp.push_back(vec[i]);
    }
    else if((vec[i]/10)>=1 && (vec[i-1]/10)<1 && vec[i-1]<=(vec[i]/10) && (vec[i]/10)<=(vec[i]%10)){
      flag2=true;
      int x=vec[i]/10;
      int y=vec[i]%10;
      dp.push_back(y);
    ...
    else if((vec[i]/10)>=1 && (vec[i-1]/10)>=1 && (vec[i-1]/10)<=(vec[i-1]%10) && (vec[i-1]%10)<=(vec[i]/10) && (vec[i]/10)<=(vec[i]%10)){
      flag2=true;
      int x=vec[i]/10;
      int y=vec[i]%10;
      dp.push_back(y);
    ...
    //   dp.push_back(vec[i]);
    // }
    // else if(vec[i-1]>(vec[i]/10) || (vec[i]/10)>(vec[i]%10)){
    //   dp.push_back(vec[i]);
    // }
  }
  if(dp[0]>=vec[0]%10 && vec[0]/10<=vec[0]%10){
    int x=vec[0]/10;
    int y=vec[0]%10;
    dp.push_back(y);
    dp.push_back(x);
