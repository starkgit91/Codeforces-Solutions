...
void solve(){
  ...
  map<vector<int>,int> v1map;
  map<vector<int>,int> v2map;
  map<vector<int>,int> v3map;
  loop(i,0,n-2,1){
    vector<int> v={vec[i],vec[i+1],vec[i+2]};
    vmap[v]++;
    v1map[{vec[i],vec[i+1]}]++;
    v2map[{vec[i+1],vec[i+2]}]++;
    v3map[{vec[i],vec[i+2]}]++;
  }
  for(auto it:v1map) count+=(it.ss*(it.ss-1))/2;
  for(auto it:v2map) count+=(it.ss*(it.ss-1))/2;
  for(auto it:v3map) count+=(it.ss*(it.ss-1))/2;
  for(auto it:vmap) count-=3*((it.ss*(it.ss-1))/2);
