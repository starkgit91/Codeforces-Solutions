...
int maxsumarrlen(vector<int> &vec,int n,int s){
  map<int,int>hmap;
  int maxsum=0,idx=0,cnt=s,k=0;
  for(int i=0;i<n;i++){
    maxsum+=vec[i];
    if(hmap.find(maxsum)==hmap.end()){
      hmap[maxsum]=i;
    }
    if(maxsum==cnt){
      if(k==0){
      ...
      else idx=max(idx,i-hmap[k]);
    } 
    if(cnt<maxsum){
      k++;
      idx=max(idx,i-hmap[k]);
      cnt=maxsum;
    }
    debug(idx)
  }
...
void solve(){
  ...
  // for(int i=0;i<n;i++){cin>>query[i].ff;query[i].ss=i;}
