...
void solve(){
  ...
  // for(int i=0;i<m;i++){cin>>query[i].ff>>query[i].ss.ff>>query[i].ss.ss;}
  ...
  map<int,int>::iterator sptr,fptr;
  sptr=hash.begin();
  // sptr++;
  debug(*sptr)
  fptr=hash.end();
  ...
  while(s<f){
    debug(*sptr)
    debug(*fptr)
    if((sptr->ff+fptr->ff)==remove){
      count+=(sptr->ss*fptr->ss);
      sptr++;
      fptr--;
      s++;
      f--;
    }
    else if((sptr->ff+fptr->ff)>remove){
      fptr--;
      f--;
    }
    else{
