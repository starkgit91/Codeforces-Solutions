...
void solve(){
  ...
  int y = (int)(log2(n));
  string bin = bitset<32>(n).to_string().substr(32-y-1);
  debug(bin)
  reverse(all(bin));
  loop(i,0,bin.size()-1,1){
  ...
  if(bin.find('0')==string::npos){
    cout<<bin.size()+1<<endl;
    cout<<-1<<" ";
    loop(i,0,bin.size()-1,1){cout<<0<<" ";}
    cout<<1<<endl;return;
  }
  ...
  int rem=sm-n;
  int x = (int)(log2(rem));
  string bin2 = bitset<32>(rem).to_string().substr(32-x-1);
  reverse(all(bin2));
  loop(i,0,bin2.size()-1,1){
    if(bin2[i]=='1' && bin2[i]==bin2[i+1]){
      flag2=true;
      break;
    }
  }
  ...
  debug(bin)
  vector<int> dp(32,0);
  for(int i=0;i<bin.size();i++){dp[i]=(bin[i]-48);}
  debug(dp)
  int j=0;
  int l=0;
  loop(i,0,dp.size()-1,1){
    if(dp[i]==1 && dp[i+1]==1){
      j=i+1;
      while(dp[j]==1 && j<32){
        j++;
