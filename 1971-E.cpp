...
int bin_search(vector<int> &dp, int key){
  int s=0;
  int f=dp.size()-1;
  while(s<=f){
    int mid = floor(s + (f-s)/2);
    ...
    else if(key<dp[mid] && mid==0){
      return mid;
    }
    else if(key<dp[mid] && key>dp[mid-1]){
      return mid;
    }
    else if(dp[mid]>key){
      f=mid;
...
void solve(){
  ...
  // for(int i=0;i<k;i++){cin>>query[i].ff>>query[i].ss;}
  ...
  loop(i,0,k,1) cin>>points[i];
  vector<int> time(k,0);
  loop(i,0,k,1) cin>>time[i];
  vector<int> query(q,0);
  loop(i,0,q,1) cin>>query[i];
  map<int,int> hash;
  loop(i,0,q,1){
    hash[query[i]]=i;
  }
  // fsort(query);
  loop(i,0,q,1){
    int x = bin_search(points,query[i]);
    debug(x)
    if(x==0){
      int speed=query[i]*time[x];
      cout<<speed/points[x]<<" ";
    }
    else{
      int speed=(query[i]-points[x-1])*(time[x]-time[x-1]);
      cout<<time[x-1]+(speed/(points[x]-points[x-1]))<<" ";
    }
  }
  cout<<"\n";
